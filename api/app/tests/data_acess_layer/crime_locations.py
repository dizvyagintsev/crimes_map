...
class TestChicagoCrimesDAL:
    def test_crime_types(self, mocked_session: bigquery.Client):
        ...
        assert ChicagoCrimesDAL(mocked_session).crime_types() == [
            "DID NOT WRITE TESTS",
            "DID NOT USE TYPING",
        ]
    ...
    def test_date_ranges(self, mocked_session: bigquery.Client):
        mocked_session.query = MagicMock(
            return_value=[
                Row((date(2001, 1, 1), date(2022, 10, 18)), {"f0_": 0, "f1_": 1})
            ],
        )

        assert ChicagoCrimesDAL(mocked_session).date_range() == DateRange(
            date(2001, 1, 1), date(2022, 10, 18)
        )
        mocked_session.query.assert_called_once_with(
    ...
    def test_locations(self, mocked_session: bigquery.Client):
        mocked_session.query = MagicMock(
            return_value=[
                Row((41.694312336, -87.631090274), {"latitude": 0, "longitude": 1})
            ],
        )

        date_range = DateRange(date(2022, 10, 1), date(2022, 10, 18))
        assert ChicagoCrimesDAL(mocked_session).crime_locations(
            date_range, ["HOMICIDE"]
        ) == [
